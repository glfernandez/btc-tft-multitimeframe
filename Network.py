...
class VSN(torch.nn.Module):
    def __init__(self, n_var_cont, n_var_disc, dim_model, dropout_r = 0.1):
        ...
        self.linearise = nn.ModuleList([
            nn.Linear(1, dim_model, bias = False) for i in range(n_var_cont)
        ])
        
        #entity embedings for discrete inputs
        self.entity_embed = nn.ModuleList([
            nn.Linear(i, dim_model, bias = False) for i in n_var_disc
        ])
...
class LSTMLayer(torch.nn.Module):
    ...
    def reset(self, batch_size, device = 'cpu'):
        """Reset LSTM hidden state for a new batch.
        ...
        """
        self.hidden = (
            torch.zeros([self.n_layers, batch_size, self.dim_model], device=device),
            torch.zeros([self.n_layers, batch_size, self.dim_model], device=device)
        )
...
class TFN(torch.nn.Module):
    def __init__(self, n_var_past_cont, n_var_future_cont, n_var_past_disc, n_var_future_disc
        ...
        self.attention = nn.ModuleList([
            nn.ModuleList([
                MultiHeadAttentionBlock(dim_model, dim_model, n_heads = n_heads),
    ...
    def reset(self, batch_size, device = 'cpu'):
        """Reset LSTM hidden states for encoder and decoder.
        
        Args:
            batch_size: Batch size
            device: Device to create tensors on ('cpu', 'cuda', 'mps')
        """
        self.enc.reset(batch_size, device)
        self.dec.reset(batch_size, device)        
