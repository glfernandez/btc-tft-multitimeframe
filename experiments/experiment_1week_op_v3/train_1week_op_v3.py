"""
Training script for experiment_1week_op_v3

This version predicts 1-week returns (percent moves) instead of raw Close.
It applies per-column normalisation (train split only) and keeps the
short-horizon forecast (2 weeks ahead).
"""
...
# CONFIGURATION
# ============================================================================
CSV_PATH = str(workspace_root / "data/resample/btcusd_1week_resampled.csv")

# Continuous features (same 14 as 1day_op_v3)
continuous_columns = [
...
n_variables_past_disc = [7, 31, 12]  # DayOfWeek: 0-6, DayOfMonth: 1-31, Month: 1-12
n_variables_future_disc = [7, 31, 12]

# Hyperparameters (mirrors 1day_op_v3)
batch_size = 160
test_batch_size = 160
dim_model = 160
n_lstm_layers = 4
...
# Sequence lengths (1-week candle)
# Note: Reduced from 80 to 40 due to limited weekly data (52 test rows)
# Need at least past_seq_len + future_seq_len + 1 = 43 rows
past_seq_len = 40   # 40 weeks lookback (reduced for weekly data)
future_seq_len = 2  # 2 weeks ahead
...
print("=" * 70)
print("EXPERIMENT 1WEEK_OP_V3 - Training Configuration")
print("=" * 70)
...
checkpoint_path = models_dir / "model_200000.pt"
...
plt.tight_layout()
loss_plot = results_dir / "training_losses_1week_op_v3.png"
