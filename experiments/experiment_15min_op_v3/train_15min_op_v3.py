"""
Training script for experiment_15min_op_v3

This version predicts 15-minute returns (percent moves) instead of raw Close.
It applies per-column normalisation (train split only) and keeps the
short-horizon forecast (30 minutes ahead).
"""
...
# CONFIGURATION
# ============================================================================
CSV_PATH = str(workspace_root / "data/resample/btcusd_15min_resampled.csv")

# Continuous features (same 14 as 4hr v3)
continuous_columns = [
...
# Hyperparameters (mirrors 4hr v3)
batch_size = 160
test_batch_size = 160
dim_model = 160
n_lstm_layers = 4
...
# Sequence lengths (15-minute candle)
past_seq_len = 80   # 1200 minutes (20 hours)
future_seq_len = 2  # 30 minutes ahead
...
print("=" * 70)
print("EXPERIMENT 15MIN_OP_V3 - Training Configuration")
print("=" * 70)
...
if checkpoint_files:
    # Filter out emergency checkpoints
    checkpoint_files = [f for f in checkpoint_files if "emergency" not in f.name]
    if checkpoint_files:
        latest = max(checkpoint_files, key=lambda p: int(p.stem.split("_")[1]) if "_" in p.stem else 0)
        try:
        ...
        print("No non-emergency checkpoints found, starting fresh.")
else:
...
plt.tight_layout()
loss_plot = results_dir / "training_losses_15min_op_v3.png"
